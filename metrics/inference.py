...
def get_frames_per_second(model, device, sequence_size, movinet, frame_size=224, iterations=10, init_iterations=2, 
        batch_size=1, use_jit=False, use_half=False):

    dtype = torch.half if use_half else torch.float32
    model = model.eval().requires_grad_(False).to(dtype).to(device)
    frames = torch.randn((batch_size * iterations, 3, sequence_size, frame_size, frame_size)).to(dtype)
    if movinet:
