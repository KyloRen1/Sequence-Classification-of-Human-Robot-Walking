...
from utils.torch import (
    train_single_epoch, evaluate_model, test_model, save_model_weights, current_datatime)
...
if __name__ == '__main__':
    ...
    train_dataset = create_dataset(
        args.train_samples_file, args.dataset_folder, 
    ...
    val_dataset = create_dataset(
        args.val_samples_file, args.dataset_folder,
        experiment_config.data_kwargs.batch_size, 
        experiment_config.data_kwargs.many_to_one_setting,
        experiment_config.data_kwargs.image_size, 
        upsample=False,
    ...
    test_dataset = create_dataset(
        args.test_samples_file, args.dataset_folder,
        experiment_config.data_kwargs.batch_size, 
        experiment_config.data_kwargs.many_to_one_setting,
        experiment_config.data_kwargs.image_size, 
        upsample=False,
    ...
    for epoch in range(experiment_config.experiment_kwargs.start_epoch, experiment_config.experiment_kwargs.end_epoch):
        train_loss, train_acc =  train_single_epoch(
            experiment_config, train_dataset, model, optimizer, 
            lr_scheduler, criterion, epoch, device)
        ...
        test_model(experiment_config, val_dataset, model, 
            device, 
            os.path.join(
                metrics_folder,
                confusion_matrix_path),
            compute_model_metrics, display_confusion_matrix)
        ...
        save_model_weights(experiment_config, model.state_dict(), args.checkpoint_dir, 
                           epoch, start_date, start_time)
