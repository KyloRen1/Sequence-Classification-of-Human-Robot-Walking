...
def train_single_epoch(config, dataloader, model, optimizer, scheduler, criterion, epoch, device):
    model.train()
...
def test_model(config, dataloader, model, device, split, metrics, plot_cm):
    ...
    if config.data_kwargs.many_to_one_setting:
        ...
        gt_arr, pred_arr = np.empty((0, config.data_kwargs.seq_len), int), np.empty((0, config.data_kwargs.seq_len), int)
    ...
    with tqdm(dataloader, unit="batch") as tqdm_epoch:
        for idx, (samples, labels) in enumerate(tqdm_epoch):
            ...
            if config.data_kwargs.many_to_one_setting:
                ...
                pred_arr = np.append(pred_arr, pred_flatten.detach().cpu().numpy(), axis=0)
    ...
    if config.data_kwargs.many_to_one_setting:
        ...
        metrics_results = 'Accuracy: {} | F1-score: {} | Precision: {} | Recall {}'.format(
        ...
        plot_cm(cmat, accuracy, f1_score, precision, recall, 
            f'{split}_confusion_matrix_{metrics_results}.png', dataloader.dataset.labels_mapping)
    else:
        print()
        print(f'========== {split} ==========')
        ...
        print(metrics_results)
        plot_cm(cmat, accuracy, f1_score, precision, recall, 
            f'{split}_confusion_matrix_M2O_{metrics_results}.png', dataloader.dataset.labels_mapping)   
        ...
        print(metrics_results)
        plot_cm(cmat, accuracy, f1_score, precision, recall, 
            f'{split}_confusion_matrix_M2M_{metrics_results}.png', dataloader.dataset.labels_mapping)   
...
def save_model_weights(config, model_state_dict, checkpoints_folder, epoch, start_date, start_time):
    ...
    folder_path = os.path.join(checkpoints_folder, 'model_weights', f'{start_date}_{start_time}')
